...
function XFrames:IsTankFramesEnabled()
	return self.db and self.db.profile and self.db.profile.raid and self.db.profile.raid.tanks and self.db.profile.raid.tanks.enabled ~= false
end
...
function XFrames:SetTankFramesEnabled(enabled)
	...
	local tankModule = self:GetModule("RaidTanks")
	if tankModule then
		tankModule.enabled = self.db.profile.raid.tanks.enabled
		if self.db.profile.raid.tanks.enabled and type(tankModule.Enable) == "function" and not tankModule.anchorFrame then
			tankModule:Enable()
		elseif type(tankModule.RefreshAll) == "function" then
			tankModule:RefreshAll()
		end
	end
...
function XFrames:IsTankTargetsEnabled()
	return self.db and self.db.profile and self.db.profile.raid and self.db.profile.raid.tanks and self.db.profile.raid.tanks.targets and self.db.profile.raid.tanks.targets.enabled ~= false
end
...
function XFrames:SetTankTargetsEnabled(enabled)
	...
	local tankModule = self:GetModule("RaidTanks")
	if tankModule and type(tankModule.RefreshAll) == "function" then
		tankModule:RefreshAll()
	end
